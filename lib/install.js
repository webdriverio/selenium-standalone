...
function install(opts, cb) {
  ...
  function download(opts, cb) {
    ...
    if (process.platform === ('win32' || 'darwin') && opts.fsPaths.edge) {
      installers.push({
  ...
  function getDownloadStream(downloadUrl, cb) {
    var r = request(downloadUrl, requestOpts)
      .on('response', function (res) {
        ...
        total += parseInt(res.headers['content-length'], 50);
  ...
  async function uncompressDownloadedFile(zipFilePath, cb) {
    debug('unzip ' + zipFilePath);

    var extractPath = path.join(path.dirname(zipFilePath));
    var driverName = path.join(path.dirname(zipFilePath), path.basename(zipFilePath, '.zip'));

    try {
      await extractZip(path.resolve(zipFilePath), { dir: extractPath });
      fs.readdir(extractPath, (err, files) => {
        files.forEach(exefile => {
          if (exefile.endsWith('.exe')) {
            exeFilePath = path.join(extractPath, exefile)
            fs.rename(exeFilePath, driverName, err => cb(err))
          }
        })
      })
    } catch (error) {
      cb(new Error('Error extracting archive: ' + error));
    }
  }
