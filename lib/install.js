    ...
    fs.createReadStream(gzipFilePath).pipe(gunzip).pipe(writeStream)
      ...
      .once('finish', function() {
        if (from.substr(-7) === '.tar.gz') {
          var destDir = path.join(os.tmpdir(), 'selenium-standalone-' + Math.random());
          var extractor = tar.Extract({path: destDir})
            ...
            .on('end', function() {
              fs.readdir(destDir, function(err, files) {
                ...
                fs.rename(path.join(destDir, files[0]), extractPath, function (err) {
                  if (err) {
                    return cb(err);
                  }
                  cb();
          ...
          return fs.createReadStream(extractPath)
            .on('error', cb.bind(null))
            .pipe(extractor);
        }
        cb()
