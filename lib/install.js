...
function install(opts, cb) {
  ...
  function downloadInstallerFile(from, to, cb) {
    if (process.platform !== "win32") {
      throw new Error(
        "Could not install an `msi` file on the current platform"
      );
    }
    ...
    getDownloadStream(from, function (err, stream) {
      ...
      msiWriteStream.once(
        "finish",
        runInstaller.bind(null, installerFile, from, to, cb)
      );
    });
  }
  ...
  function installGzippedFile(from, to, cb) {
    getDownloadStream(from, function (err, stream) {
      ...
      gzipWriteStream.once(
        "finish",
        uncompressGzippedFile.bind(null, from, to, cb)
      );
    });
  }
  ...
  function uncompressGzippedFile(from, gzipFilePath, cb) {
    ...
    var extractPath = path.join(
      path.dirname(gzipFilePath),
      path.basename(gzipFilePath, ".gz")
    );
    var writeStream = fs
    ...
    var gunzippedContent = fs
      ...
      .once(
        "error",
        cb.bind(null, new Error("Could not read " + gzipFilePath))
      );
      ...
      downloadStream
        ...
        .on("downloadProgress", ({ transferred, total, percent }) => {                                 
          ...
          if (expectedRequests === startedRequests) {            
            opts.progressCb(parseInt(total), parseInt(transferred), chunk);
          }        
        })
        .on("finish", () => {
        ...
        .on("error", (error) => {          
          return cb(new Error(`Could not download ${downloadUrl}. Error: ${error}`))
        });      
    })();    
  }
  ...
  function uncompressDownloadedFile(zipFilePath, cb) {
    ...
    yauzl.open(zipFilePath, function onOpenZipFile(err, zipFile) {
      ...
      zipFile.on("entry", function (entry) {
        ...
        zipFile.openReadStream(entry, function onOpenZipFileEntryReadStream(
          err,
          readStream
        ) {
          if (err) {
          ...
          readStream
            ...
            .once(
              "error",
              cb.bind(null, new Error("Could not read " + zipFilePath))
            )
...
function logInstallSummary(logger, paths, urls) {
  ["selenium", "chrome", "ie", "firefox", "edge", "chromiumedge"].forEach(
    function log(name) {
      if (!paths[name]) {
        return;
      }
  ...
  (async () => {
      try {
          ...
          if(res.statusCode !== 200) {
            return cb(new Error(`Error processing request for ${url}: ${res.statusCode}`));
          }
          return cb(null, false)
      } catch (error) {          
          return cb(new Error(`Unable to connect to ${url}. Error: ${error}`));
      }
  })();  
}
