...
function install(opts, cb) {
  ...
  function uncompressGzippedFile(from, gzipFilePath, cb) {
    ...
    var writeStream = fs.createWriteStream(extractPath).once('error', function () {
  ...
  function installZippedFile(from, to, cb) {
    getDownloadStream(from, function (err, stream) {
      ...
      // Store downloaded compressed file
      var zipWriteStream = fs.createWriteStream(to).once('error', cb.bind(null, new Error('Could not write to ' + to)));
  ...
  function uncompressDownloadedFile(zipFilePath, cb) {
    ...
    yauzl.open(zipFilePath, function onOpenZipFile(err, zipFile) {
      ...
      zipFile.on('entry', function (entry) {
        ...
        zipFile.openReadStream(entry, { autoClose: true }, function onOpenZipFileEntryReadStream(err, readStream) {
          if (err) {
            cb(err);
            return;
          }
