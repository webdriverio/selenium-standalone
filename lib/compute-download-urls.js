...
function computeDownloadUrls(opts, askedOpts) {
  ...
  if (opts.drivers.edge) {
    downloadUrls.edge = util.format(
      ...
      opts.drivers.edge.version,
      getEdgeDriverArchitecture(opts.drivers.edge.arch)
    );
  }
...
function getEdgeDriverArchitecture(wantedArchitecture) {
  var platform;
  
  if (process.platform === 'darwin') {
    platform = 'mac64';
  } else if(process.platform === 'win32') {
    platform = (wantedArchitecture === 'ia32') ? 'win32': 'win64';
  }
  return platform;
}
