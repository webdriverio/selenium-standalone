...
async function computeDownloadUrls(options) {
  ...
  if (opts.drivers.chrome) {
    if (opts.drivers.chrome.version === 'latest' && !opts.drivers.chrome.fullURL) {
      ...
      opts.drivers.chrome.baseURL = opts.drivers.chrome.baseURL ? opts.drivers.chrome.baseURL : 'https://chromedriver.storage.googleapis.com';
      await resolveLatestVersion(
        opts,
        'chrome',
        opts.drivers.chrome.baseURL + oldVersion ? `/LATEST_RELEASE_${oldVersion}` : '/LATEST_RELEASE'
      );
