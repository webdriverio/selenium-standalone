...
const { validateMajarVersionPrefix, getVersionWithZeroedPatchPart } = require('./validation');
...
async function computeDownloadUrls(opts) {
  ...
  if (opts.drivers.chrome) {
    if (opts.drivers.chrome.version === 'latest' && !opts.drivers.chrome.fullURL) {
      opts.drivers.chrome.version = await resolveBuildId('chromedriver', detectBrowserPlatform(), opts.drivers.chrome.channel ? opts.drivers.chrome.channel : 'stable');
      downloadUrls.chrome = resolveDownloadUrl(detectBrowserPlatform(), opts.drivers.chrome.version, opts.drivers.chrome.baseURL);
    } else if (validateMajarVersionPrefix(opts.drivers.chrome.version) && Number(validateMajarVersionPrefix(opts.drivers.chrome.version)) > 114 && !opts.drivers.chrome.fullURL) {
      const lastVersionFromMajor = opts.drivers.chrome.major ? opts.drivers.chrome.major : await getLastChromedriverVersionFromMajor(opts.drivers.chrome.version);

      if (lastVersionFromMajor) {
        const url = lastVersionFromMajor.downloads.chrome.map(m => {
        ...
        if (url.length) {
          ...
          opts.drivers.chrome.version = lastVersionFromMajor
        } else {
      ...
      opts.drivers.chrome.baseURL = 'https://chromedriver.storage.googleapis.com'
...
async function getLastChromedriverVersionFromMajor(version) {
  ...
  const versionsWithMajor = response.data.versions.filter(f => validateMajarVersionPrefix(f.version) === validateMajarVersionPrefix(version) && 'chromedriver' in f.downloads);
  versionsWithMajor.sort((version1, version2) => {
