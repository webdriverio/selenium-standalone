...
function getConfigProcessesName(drivers) {
  ...
  if (driversName && driversName.length) {
    for (let driverName of driversName) {
...
async function processKiller(drivers, portValue) {
  if (portValue) {
    const portCast = `${portValue}`.startsWith(':') ? `${portValue}`.substring(1) : `${portValue}`;

    if (!Number.isNaN(Number(portCast))) {
...
async function killProcessByCmd(processes, type) {
  if (processes && processes.length) {
    for (let processSingle of processes) {
