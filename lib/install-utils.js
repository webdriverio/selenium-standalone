...
async function setDriverFilePermissions(where) {
  ...
  const requireChmod = await gotAccess(where)
...
async function gotAccess(where) {
  try{
    fsPromises.access(where, 
      fs.constants.R_OK | fs.constants.W_OK)
    .then(() => {debug('Can be accessed to driver files');
    return true;
    })
  .catch(() => {
    console.error('Can not be accessed to driver files')
    return false;
  });
  } catch (err) {
...
async function checksum(filepath, cb) {
